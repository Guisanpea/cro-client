...
import faith.knowledge.common.ChainingSyntax.scalaUtilChainingOps
  ...
  "A sig provider with a given key and pass" should {
    ...
    "encode properly a given request" in {
      ...
      val request = CreateOrderRequestParams(
        ...
        side = side,
        `type` = `type`
      ) pipe (params => CroPrivateRequest(id = id, method = method, nonce = nonce, params = params.some))
      val noHashSig = s"$method$id${apiKey}instrument_name${instrumentName}notional${notional}side${side}type${`type`}$nonce"
      val expectedSig = (
        noHashSig
